...
const lastRecord = (() => { // Estado de la última partida
...
      window.console.error('Error al leer el registro:', e);
    }
    return false;
  }
  return data;
...
const eachLines = 20; // Por cada 'eachLines' eliminadas, la velocidad aumenta

const getParam = (param) => { // Obtener parámetro de la URL
  const r = new RegExp(`\\?(?:.+&)?${param}=(.*?)(?:&.*)?$`);
  const m = window.location.toString().match(r);
  return m ? decodeURI(m[1]) : '';
};

// --- CORRECCIÓN FINAL: FORZAR EL IDIOMA A 'es' ---
const lan = 'es'; 
// ---------------------------------------------------

document.title = i18n.data.title[lan]; // Se usa 'es' para el título
